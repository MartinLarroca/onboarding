...
const findAll = async () => {
  try {
    const tweets = await Database.tweets.findAll({raw: true});
    return tweets;
  } catch (err) { return {message : err }}
};
...
const findOne = async (id: string) => {
  try {
    const tweet = await Database.tweets.findByPk(id, {raw: true});
...
const del = async (id: string) => {
  try {
    ...
    return elim == 0? { message: `Id ${id} is not valid.` } : { success: "Tweet deleted successfully." }
  } catch (err) { return { message : err }}
};
